...
// compareViewContents wraps this package's exported View-content comparator
// (CheckViewsEquivalent, viewcheck.go) in the t.Fatal-on-error shape this
// test file's callers expect. The comparison itself -- by database id
// rather than dense id, since a folded Snapshot renumbers everything while
// a stacked overlay View keeps tombstone gaps and virtual ids -- lives in
// viewcheck.go so a caller outside this package (the engine's own compactor
// tests) can reuse it too.
func compareViewContents(t *testing.T, a, b *View) {
	t.Helper()
	if err := CheckViewsEquivalent(a, b); err != nil {
		t.Fatal(err)
	}
}
