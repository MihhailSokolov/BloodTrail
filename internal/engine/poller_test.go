...
// suppresses retrying the same stamp, retrying resumes once the stamp
// itself advances, and a remembered refusal only suppresses rule (b) -- rule
// (c) still independently fires when its own condition holds.
func TestDecideRebuild(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:   "remembered refusal suppresses rule (b) but rule (c) still fires",
			st:     &pollState{lastStamp: t1, refused: true},
			status: "idle",
			stamp:  t1,
			snap:   &snapshot.Snapshot{AnalysisStamp: t0},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := decideRebuild(tc.st, tc.status, tc.stamp, tc.snap, tc.fresh)
			if got != tc.want {
...
// it has already decided (via decideRebuild) to rebuild for, including the
// same remembered-refusal case TestDecideRebuild exercises: rule (b)'s
// textual condition can hold even when a remembered refusal means rule (c)
