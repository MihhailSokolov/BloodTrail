...
//
// The size/row-count match is NOT skipped along with it when the comparison
...
func evaluateShape(btP50, pgP50 time.Duration, matchChecked, match, pgCapped bool, th shapeThreshold) (ok bool, reasons []string) {
	if pgCapped {
		...
		if matchChecked && !match {
			reasons = append(reasons, "bt/pg size metric mismatch (measured at warmup, before the pg baseline was capped)")
		}
